...
function buildBaselineSuggestions(opts: {
  ...
  // NEW: category name -> id lookup for baseline suggestions
  ...
  if (month === 1) {
    ...
    suggestions.push({
      ...
      dismissed: false,
      createdAt: now,
      categoryId: categoryIds["Health"], // awareness related to Health
    });
  }
  ...
  if (hasHealthCategory && !hasHealthTask) {
    suggestions.push(
      ...
      },
      {
        ...
        title: "Check Mental Health Benefits",
        message:
    ...
    suggestions.push(
      {
        ...
        dismissed: false,
        createdAt: now,
        categoryId: categoryIds["Warranties"],
      },
      {
        id: "baseline-warranties-quick-win",
        title: "Save Receipts & Serials",
        message:
    ...
    suggestions.push(
      {
        ...
        title: "Change HVAC/Furnace Filter",
        message:
      ...
      },
      {
        ...
        title: "Mortgage Renewal Awareness",
        message:
        ...
        dismissed: false,
        createdAt: now,
        categoryId: categoryIds["Taxes & Finance"], // or Home Maintenance if preferred
      }
    );
  }
  ...
  if (hasTaxesCategory && !hasTaxesTask) {
    suggestions.push({
      ...
      title: "Start Tax Document Folder",
      message:
      ...
      dismissed: false,
      createdAt: now,
      categoryId: categoryIds["Taxes & Finance"],
    });
  }
...
export const generateCategoryBasedSuggestions = (
  ...
  // NEW: build category name -> id map
  const categoryIds = categories.reduce<Record<string, string | undefined>>(
    (acc, c) => {
      acc[c.name] = c.id;
      return acc;
    },
    ...
    if (category?.name === "Vehicle") {
      ...
      if (
        ...
        suggestions.push({
          ...
          title: "Renew Vehicle Insurance/Registration",
    ...
    if (category?.name === "Subscriptions") {
      ...
      if (daysUntil >= -7 && daysUntil <= 7) {
        suggestions.push({
          ...
          title: "Subscription Renewal Window",
      ...
      if (daysUntil > 7 && daysUntil <= 30) {
        suggestions.push({
          ...
          title: "Review Subscription Plan",
    ...
    if (category?.name === "Warranties") {
      ...
      if (daysUntil > 30 && daysUntil <= 90) {
        suggestions.push({
          ...
          title: "Warranty Ending in Months",
      ...
      if (daysUntil < 0 && daysUntil >= -30) {
        suggestions.push({
          ...
          title: "Warranty Recently Expired",
